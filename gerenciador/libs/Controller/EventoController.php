...
class EventoController extends AppBaseController
{
	...
	public function Query()
	{
		try
		{
			...
			if ($filter) $criteria->AddFilter(
				new CriteriaFilter('Id,Nome,Local,Endereco,Observacao,Data,IdCidade,Privado,Link,Map,IdTipoEvento'
				, '%'.$filter.'%')
			);
	...
	public function Create()
	{
		try
		{
			...
			$evento->Endereco = $this->SafeGetVal($json, 'endereco');
			$evento->Observacao = $this->SafeGetVal($json, 'observacao');
			$evento->Data = date('Y-m-d H:i:s',strtotime($this->SafeGetVal($json, 'data')));
			$evento->IdCidade = $this->SafeGetVal($json, 'idCidade');
			$evento->Privado = $this->SafeGetVal($json, 'privado');
			$evento->Link = $this->SafeGetVal($json, 'link');
			$evento->Map = $this->SafeGetVal($json, 'map');
			$evento->IdTipoEvento = $this->SafeGetVal($json, 'idTipoEvento');
	...
	public function Update()
	{
		try
		{
			...
			$evento->Endereco = $this->SafeGetVal($json, 'endereco', $evento->Endereco);
			$evento->Observacao = $this->SafeGetVal($json, 'observacao', $evento->Observacao);
			$evento->Data = date('Y-m-d H:i:s',strtotime($this->SafeGetVal($json, 'data', $evento->Data)));
			$evento->IdCidade = $this->SafeGetVal($json, 'idCidade', $evento->IdCidade);
			$evento->Privado = $this->SafeGetVal($json, 'privado', $evento->Privado);
			$evento->Link = $this->SafeGetVal($json, 'link', $evento->Link);
			$evento->Map = $this->SafeGetVal($json, 'map', $evento->Map);
			$evento->IdTipoEvento = $this->SafeGetVal($json, 'idTipoEvento', $evento->IdTipoEvento);
