...
class ConsultarEventos{
	...
	private function ConsultarListaEventos($pnIdCidade, $pbEmailValido){
		...
		$sSql = "SELECT e.id, e.nome, e.local, e.endereco, e.observacao, e.data, e.privado, e.id_cidade, c.nome as nome_cidade ".
		"FROM eventos e join cidades c on c.id = e.id_cidade ".
		"WHERE e.id_cidade = ? ";
