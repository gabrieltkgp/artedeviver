<?php
	$this->assign('title','gerenciador | Cidades');
	...
	<script type="text/template" id="cidadeCollectionTemplate">
		...
		<thead>
			<tr>
				...
				<th id="header_IdEstado">Id Estado<% if (page.orderBy == 'IdEstado') { %> <i class='icon-arrow-<%= page.orderDesc ? 'up' : 'down' %>' /><% } %></th>
			</tr>
		</thead>
		<tbody>
		<% items.each(function(item) { %>
			<tr id="<%= _.escape(item.get('id')) %>">
				...
				<td><%= _.escape(item.get('idEstado') || '') %></td>
			</tr>
		<% }); %>
		</tbody>
		</table>
	...
	<script type="text/template" id="cidadeModelTemplate">
		<form class="form-horizontal" onsubmit="return false;">
			<fieldset>
				...
				<div id="idEstadoInputContainer" class="control-group">
					<label class="control-label" for="idEstado">Id Estado</label>
					<div class="controls inline-inputs">
						<input type="text" class="input-xlarge" id="idEstado" placeholder="Id Estado" value="<%= _.escape(item.get('idEstado') || '') %>">
						<span class="help-inline"></span>
					</div>
				</div>
			</fieldset>
